namespace CRC32
{
	...
	unsigned int ToCRC32(std::string Sz)
	{
		unsigned int Hash = CRC32((void*)Sz.data(), Sz.length());
		return Hash;
		//	std::cout << "0x" << std::uppercase << std::hex << Hash << " -> " << Sz.c_str() << std::endl;
	}
}
...
#define HASH( str )                                     \
    []() {                                              \
        constexpr Hash::hash32_t out{ Hash::FNV1a::Get( str ) };    \
                                                        \
        return out;                                     \
    }()

// compile-time const hash.
#define CONST_HASH( str ) Hash::FNV1a::Get( str )
